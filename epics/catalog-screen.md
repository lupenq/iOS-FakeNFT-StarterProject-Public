...
- [ ] `UITableView` отображает коллекции: обложка, название, количество NFT
- [ ] Данные загружаются с сервера, во время загрузки виден `UIActivityIndicator`
- [ ] Сортировка работает и сохраняется в `UserDefaults` между запусками
- [ ] Тап по ячейке открывает экран коллекции NFT

### Экран коллекции NFT

- [ ] Отображаются обложка, название, описание, ссылка на автора
- [ ] Сайт автора открывается в `WKWebView`
- [ ] `UICollectionView` показывает NFT: изображение, название, рейтинг, цена в ETH
- [ ] Работают кнопки избранного и корзины (крестик, если NFT в корзине)
- [ ] Тап по NFT открывает экран деталей NFT

### Архитектура

- [ ] MVVM: View / ViewModel / Service разделены на обоих экранах
